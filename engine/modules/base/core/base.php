...
class base {
    ...
    public function addModifiers() {
        ...
        // Добавляем свой модификатор в шаблонизатор для ограничения кол-ва предложений
        $this->tpl->addModifier('sentence', function ($data, $limit) {
            return bpModifiers::sentenceLimit($data, $limit);
        });

        // Добавляем свой модификатор в шаблонизатор для вывода картинок
        $this->tpl->addModifier('image', function (
            $data,
            $noimage = '',
            $imageType = 'small',
            $number = 1,
            ...
            $showSmall = false,
            $subdir = false
        ) use ($configForResizer) {
            return bpModifiers::getImage($data, $noimage, $imageType, $number, $size, $quality, $resizeType,
                $grabRemote, $showSmall, $subdir, $configForResizer);
        });

        // Добавляем свой модификатор в шаблонизатор для вывода картинок через tinypng
        $this->tpl->addModifier('tinypng', function (
            $data,
            $noimage = '',
            $imageType = 'small',
            $number = 1,
            ...
            return bpModifiers::getImage($data, $noimage, $imageType, $number, $size, $quality, $resizeType,
                $grabRemote, $showSmall, $subdir, $configForResizer, 'tinypng');
        });

        // Добавляем свой модификатор в шаблонизатор для вывода картинок через kraken
        $this->tpl->addModifier('kraken', function (
            $data,
            $noimage = '',
            $imageType = 'small',
            $number = 1,
            ...
            return bpModifiers::getImage($data, $noimage, $imageType, $number, $size, $quality, $resizeType,
                $grabRemote, $showSmall, $subdir, $configForResizer, 'kraken');
        });
        ...
        $this->tpl->addModifier('getAuthors', function ($data, $fields = false) use ($db) {
        ...
        if ($this->dle_config['allow_tags'] && $tags) {
            ...
            foreach ($tags as $value) {
                ...
                if ($this->dle_config['allow_alt_url'] && $this->dle_config['allow_alt_url'] != 'no') {
                    $showTagsArr[] = "<a href=\"".$this->dle_config['http_home_url']."tags/".urlencode($value)."/\">"
                        .$value."</a>";
                } else {
                    $showTagsArr[] = "<a href=\"".$this->dle_config['http_home_url']."?do=tags&amp;tag="
                        .urlencode($value)."\">".$value."</a>";
                }
    ...
    foreach ($cat_list as $cat) {
        if (isset($cat_info[$cat])) {
            ...
            if ($config['allow_alt_url'] && $config['allow_alt_url'] != 'no') {
                $my_cat_link[] = '<a href="'.$config['http_home_url'].get_url($cat_info[$cat]['id']).'/">'
                    .$cat_info[$cat]['name'].'</a>';
            } else {
                $my_cat_link[] = '<a href="'.$config['http_home_url'].'?do=cat&category='.$cat_info[$cat]['alt_name']
                    .'">'.$cat_info[$cat]['name'].'</a>';
            }
        }
    }
...
/**
 * Показ рейтинга через модуль. Функция переписана из стандартной т.к. стандартная имела ID, конфликтующие с самими
 * собой.
 *
...
HTML;
        ...
        if ($allow) {
            $rated
                = "<span data-rating-layer=\"{$id}\" class=\"ignore-select\" ><span class=\"ratingtypeplus ignore-select\" >{$rating}</span></span>";
        } else {
        ...
        if ($allow) {
            $rated
                = "<span data-rating-layer=\"{$id}\" class=\"ignore-select\" ><span class=\"ratingtypeplusminus ignore-select {$extraclass}\" >{$rating}</span></span>";
        } else {
