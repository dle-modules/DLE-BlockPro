		...
		<div class="col col-mb-12">
			...
			<?php if (count($arVidgets) > 0): ?>
				<?php foreach ($arVidgets as $key => $vidgetItem): ?>
					...
					unset($params['kraken_secret'], $params['tinypng_key'], $params['kraken_key']);
					...
					<div class="widget-item content">
						<div class="col col-mb-9">
							<b class="text-muted">[<?php echo $vidgetItem['id'] ?>]</b>
							<h3 class="m0 mr10 fz20 d-ib"><a
										href="/blockpro.php?block=<?php echo $vidgetItem['block_id'] ?>"
										target="_blank"><?php echo $vidgetItem['name'] ?></a></h3>
							(создан <?php echo $vidgetItem['date'] ?>) <a
									href="?mod=blockpro&setPreview=y&<?php echo $vidgetModuleUrlParams ?>#tab2"
									class="btn btn-small"
									title="Предпросмотр виджета или создание нового на основе текущего">Предпросмотр</a>
						</div>
						<div class="col col-mb-3 ta-right">
							<span class="btn btn-small delete-widget" data-widget-action="<? $PHP_SELF ?>?mod=blockpro"
							      data-widget-id="<?php echo $vidgetItem['id'] ?>"
							      data-widget-name="<?php echo $vidgetItem['name'] ?>">удалить</span>
						</div>
						<div class="col col-mb-12">
				...
				<?php endforeach ?>
			<?php else: ?>
				<div class="alert alert-info">Вы пока не создали ни одного виджета. Если вы только что его создали -
					нажмите F5 т.к. виджет создаётся без перезагрузки страницы.
				</div>
			<?php endif ?>
		</div>
	</div> <!-- .content -->
